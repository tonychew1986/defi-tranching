// SPDX-License-Identifier: MIT
// test scenario for staking and unstaking

const { expectRevert } = require('@openzeppelin/test-helpers');
const { deployProxy } = require('@openzeppelin/truffle-upgrades');
const { assert, expect } = require("chai");
const { promisify } = require("util");
const { toWei, getBalance,  nutmeg_factory, mint_test_tokens,
	BAD_TOKEN, stake: stake, log, report, fromWei, report_vtb,
	latestBlock, advanceBlockBy, report_balances } = require('./utils');
const { start_baseline, end_baseline } = require('./test-report');

describe('nut-distrib-large', ()=> {
before (async() => {
  start_baseline('nut-distrib-large')
});
after(async() => {
  end_baseline()
});

/// A run through for a borrow/staking scenario
contract("Nut distribution test", addresses => {
  const [owner, governor, p1, p2, p3, b1, b2, blackhat, _] = addresses;
  let nutmeg, token, pos, ctoken, nut;
  before(async() => {
    [nutmeg, token_contract, ctoken_contract, compound_adapter,
     nut_distributor_contract, nut_contract,
     oracle_contract] =
      await nutmeg_factory(owner, governor, p1, p2, p3, b1, b2);
    token = token_contract.address;
    ctoken = ctoken_contract.address;
    nut = nut_contract.address;
    oracle_contract.setPrices([
      token
    ], [
      toWei(1800.0)
    ]);
    await Promise.all([
      token_contract.mint(p1, toWei(100000000000)),
      token_contract.mint(p2, toWei(200)),
      token_contract.mint(p3, toWei(100000000000)),
      token_contract.approve(nutmeg.address, toWei(14000000000), {from: p1}),
      token_contract.approve(nutmeg.address, toWei(100), {from: p2}),
      token_contract.approve(nutmeg.address, toWei(10000000000), {from: p3})
    ]);
  });
  it("Nut distribution test", async() => {
    await log(async () => {
      await stake(nutmeg, token, 0, toWei(10000000000), p1)
    }, "p1 stakes 10000000000 ETH");
    await log(async () => {
      await stake(nutmeg, token, 0, toWei(50), p2)
    }, "p2 stakes 50 ETH");
		await log(async () => {
      await stake(nutmeg, token, 0, toWei(5000000000), p3)
    }, "p3 stakes 5000000000 ETH");
    await report_balances(
      token, 'base_token',
      [nutmeg.address, ctoken, p1, p2, p3],
      ['nutmeg', 'ctoken', 'p1', 'p2', 'p3']
    );

    await report_balances(
      nut, 'nut_token',
      [nut, p1, p2, p3],
      ['nut', 'p1', 'p2', 'p3']
    );
  });
  it("Advance blocks", async() => {
    await advanceBlockBy(500);
    console.log('advance blocks by 500');
    console.log((await latestBlock()).toString());
  }).timeout(600000);
  it("Distribute nutmeg", async() => {
    await nut_distributor_contract.distribute({from: governor});
    await report_balances(
      nut, 'nut_token',
      [nut, p1, p2, p3],
      ['nut', 'p1', 'p2', 'p3']
    );
    await report(
      () => [
        nut_distributor_contract.getCollectionAmount({from: p1}),
        nut_distributor_contract.getCollectionAmount({from: p2}),
				nut_distributor_contract.getCollectionAmount({from: p3})
      ],
      ['collection p1:',
       'collection p2:',
			 'collection p3:'
      ]
    );
    await nut_distributor_contract.collect({from: p1});
    await nut_distributor_contract.collect({from: p2});
		await nut_distributor_contract.collect({from: p3});
    await report_balances(
      token, 'base_token',
      [nutmeg.address, ctoken, p1, p2, p3],
      ['nutmeg', 'ctoken', 'p1', 'p2', 'p3']
    );
    await report_balances(
      nut, 'nut_token',
      [nut, p1, p2, p3],
      ['nut', 'p1', 'p2', 'p3']
    );
  });
  // tests for issue #141
  // needs many awaits to trigger issue
  it("unstake", async() => {
    await log(async() => {
      await nutmeg.unstake(token, 0, toWei(5000000000), {from: p1})
    }, "p1 unstake attempt 50 ETH");
		await log(async () => {
      await stake(nutmeg, token, 0, toWei(2000000000), p1)
    }, "p2 stakes 50 ETH");

    await log(async () => {
      await nutmeg.unstake(token, 0, toWei(7000000000), {from: p1})
    }, "p1 unstake attempt 50 ETH");
    await log(async () => {
      await nutmeg.unstake(token, 0, toWei(50), {from: p2})
    }, "p2 unstake attempt 50 ETH");
		await log(async () => {
      await nutmeg.unstake(token, 0, toWei(5000000000), {from: p3})
    }, "p2 unstake attempt 50 ETH");
    await report_vtb(nut_distributor_contract, token, p1);
  });
});
});

/* Local Variables:   */
/* mode: javascript   */
/* js-indent-level: 2 */
/* End:               */
